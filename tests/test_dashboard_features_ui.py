...
def test_trend_sparkline_renders_something(authenticated_session, base_url):
    """
    The scheduled job polls immediately on app startup, so by the time
    this test runs there should be at least one snapshot for the default
    pair shown on the Overview panel. This checks that SOMETHING rendered
    (a chart) rather than asserting exact data, since real FX rates vary.
    """
    dashboard = DashboardPage(authenticated_session, base_url)
    dashboard.open_dashboard()
    ...
    if not has_chart:
        # Acceptable alternate state — no snapshot yet for this exact pair
